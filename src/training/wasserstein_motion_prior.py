...
def batched_convol_bary_debiased(
        ...
        n_per_eps=5, n_extra_last_scale=15,
...
class FastConvolutionalW2Cost(_Loss):
    def __init__(
            self, reduction='mean', scaling=0.95, use_pykeops=False, n_per_eps=5, n_extra_last_scale=15,
    ):
...
if __name__ == '__main__':
    ...
    def create_circle(_translation, inner_rad=3., val=0.5):
        res = 32
        _image_of_circle = torch.zeros(res, res)
        ...
        _circle_outer = ((_x - 24 - _translation) ** 2 + (_y - 24 - _translation) ** 2 <= 3 ** 2) & (
                (_x - 24 - _translation) ** 2 + (_y - 24 - _translation) ** 2 > inner_rad ** 2)
        ...
        _circle_outer = ((_x - 8 - _translation) ** 2 + (_y - 8 - _translation) ** 2 <= 3 ** 2) & (
                (_x - 8 - _translation) ** 2 + (_y - 8 - _translation) ** 2 > inner_rad ** 2)
        _image_of_circle[_circle_inner] = 1
        _image_of_circle[_circle_outer] = val
    ...
    _image_of_circle = create_circle(0., inner_rad=1.5, val=0.5)

    b_s = 1  # batch size
    rm = 1  # resolution multiplier

    _source = _image_of_square.unsqueeze(0).repeat(b_s, 1, rm, rm).cuda()
    # _source = torch.rand(1, 1, 32, 32, dtype=torch.float32, device='cuda')
    _source.requires_grad_(True)
    _target = _image_of_circle.unsqueeze(0).repeat(b_s, 1, rm, rm).cuda()

    _loss = FastConvolutionalW2Cost(
        scaling=0.5, reduction='none', n_per_eps=5, n_extra_last_scale=15, use_pykeops=False
    )

    _ = _loss(_source, _target)  # warm up
    _start = perf_counter_ns()
    _cost = _loss(_source, _target)
    _end = perf_counter_ns()

    print(f"Cost: {_cost.mean().item()}")
    print(f"Time taken: {(_end - _start) / 1e6} ms")
    ...
    plt.colorbar(axs[0].imshow(_g[0].squeeze().detach().cpu().numpy()), ax=axs[0])
    axs[0].set_title("Gradient")
    axs[1].imshow(_source[0].squeeze().detach().cpu().numpy())
    axs[1].set_title("Source")
    axs[2].imshow(_target[0].squeeze().cpu().numpy())
