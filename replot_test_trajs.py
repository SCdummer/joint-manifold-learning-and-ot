...
def save_individual_images(time_series_recon, save_dir):
    ...
    for i in range(time_series_recon.shape[0]):
        fig, ax = plt.subplots()
        max_val = time_series_recon[i, ...].max() * 0.0 + 1.0 * 0.7
...
def evaluate_model_on_time_series(encoder, decoder, time_warper, time_series_input, save_dir, recon_idx,
                                  nabla_t, num_int_steps):

    # Get the input time series
    img0 = time_series_input[0][None, ...]
    time_series_length = time_series_input.size(0)

    # Get the latents of the initial image
    _, z0, _ = encoder(img0)

    # Get a latent time series
    t = torch.linspace(0, (time_series_length - 1) * nabla_t,
                        (time_series_length - 1) * num_int_steps + 1)
...
def evaluate_model_on_full_dataset(experiment_directory, specs, save_dir, split='train'):
    ...
    test_data_source = os.path.join(os.path.dirname(__file__), specs.get("TestDataSource", specs["DataSource"]))
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(main_dir, args.experiment_directory, 'some_final_stuff_for_submission')
