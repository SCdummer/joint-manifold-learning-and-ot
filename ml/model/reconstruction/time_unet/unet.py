...
from ml.model.reconstruction.time_unet.nn import (
    avg_pool_nd,
    checkpoint,
    conv_nd,
    linear,
...
@utils.register_model(dataset='HeLa', name='t_unet')
class HeLaTimeUNet(UNetModelWrapper):

    def __init__(self, dim=(1, 64, 64), num_channels=32, num_res_blocks=1, channel_mult=(1, 2, 3, 4)):
        super().__init__(dim=dim, num_channels=num_channels, num_res_blocks=num_res_blocks, channel_mult=channel_mult)

    def forward(self, x):
        return super().forward(th.zeros(x.shape[0], device=x.device), x, None)


if __name__ == '__main__':
    _model = HeLaTimeUNet()
    _x = th.randn(2, 1, 64, 64)
    _t = th.zeros(2)
    _y = _model(_t, _x)
    print(_y.shape)
