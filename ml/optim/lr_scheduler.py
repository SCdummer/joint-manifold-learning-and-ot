...
def get_cosine_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            ...
            return lr_min + (0.5 * (1 + math.cos(2 * math.pi * (current_step - num_warmup_steps) / (
                    num_training_steps - num_warmup_steps) * num_cycles)) * (1 - lr_min))

    return LambdaLR(optimizer, lr_lambda, last_epoch)


def plot_scheduler(lr_scheduler, num_training_steps):
    ...
    plt.hlines(y[0], 0, num_training_steps, colors='r', linestyles='dashed', label=f'lr={y[0][0]}')
    plt.hlines(y[-1], 0, num_training_steps, colors='g', linestyles='dashed', label=f'lr={y[-1][0]:.4f}')
    plt.hlines(max(y), 0, num_training_steps, colors='b', linestyles='dashed', label=f'lr={max(y)[0]}')
